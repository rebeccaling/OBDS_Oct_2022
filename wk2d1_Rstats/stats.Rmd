...
> Answer: randomly generate. if we increased the size of sample
...
```{r}
#breaks says how many divisions in histogram
hist(y, breaks=20)

#can do it in ggplot too - but need dataframe
z<-as.data.frame(y)
z
...
df_stats <-
  ...
  gather(key = key, value = value, mean:median:standard_dev)
...
ggplot(z, aes(y))+
  geom_histogram()+
  geom_vline(aes(xintercept = 9.88, color = 'red'))
...
p2<- seq(-5,5,0.1)
norm_plot <- data.frame(x=p2, cdf=pnorm(p2))

ggplot(norm_plot)+
  geom_line(aes(x=p2, y=cdf))+
  geom_point(aes(x=p2, y=cdf))
```
...
```{r}
p3<- seq(0,1,0.01)
norm_inverse_plot <- data.frame(x=p3, cdf=qnorm(p3))

ggplot(norm_inverse_plot)+
  geom_line(aes(x=p3, y=cdf))+
  geom_point(aes(x=p3, y=cdf))
```
...
```{r}
p4<- seq(-5,5,0.1)
norm_density_plot <- data.frame(x=p4, cdf=dnorm(p4))

ggplot(norm_density_plot)+
  geom_line(aes(x=p4, y=cdf))+
  geom_point(aes(x=p4, y=cdf))
...
#based on our data tell us what the cumulative distribution less than or equal to this number eg 6.6
ecdf_iris_sepal_length(6.6)
...
cell_info <- 
read_excel('/project/obds/shared/resources/3_r_stats_genomics/statistics/data/GSE111543.xlsx', sheet='cell_info')
...
#subset cells with mock infection and infection
cells_mock = cell_info$Sample[cell_info$Infection=='Mock']
cells_infected =cell_info$Sample[cell_info$Infection!='Mock']

gene_1_mock <- logcounts_matrix[1,cells_mock]
gene_1_infected <- logcounts_matrix[1,cells_infected]
head(gene_1_mock)

out <- t.test(gene_1_mock, gene_1_infected)
out$p.value
...
t_test_row <- function(index){
  ...
  out <- t.test(gene_mock, gene_infected)
 return(out$p.value)
  }
