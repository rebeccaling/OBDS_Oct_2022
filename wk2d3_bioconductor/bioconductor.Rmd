...
> Answer: 41 out of date at the bottom of the message there is code which will update everything.
> 
...
#lower case letters are higher quality data.
#typically start of sequencing is of poorer quality.
```
...
```{r}
#file.edit shows what the file looks like. 
...
countPattern('CANNTG', chrY, fixed=F)

countPattern(Ebox, chrY, fixed=F)

#we use fixed =F to allow for N not matching a base.
```
...
```{r}
human_symbols <- head(keys(org.Hs.eg.db, keytype='SYMBOL'), n=8)
#changing n gives different number of column
...
> Answer:
> Problem if you load tidyverse after Bioconductor
...
#gives a character vector of the same length as original vector
#select gives all information - but may not be the same size
```
...
colnames(assay_counts)[1] <- 'gene'

rownames(assay_counts) <-  assay_counts$gene
head(assay_counts)
assay_counts<- as.matrix(assay_counts)
dim(assay_counts)
assay_counts <- assay_counts[,2:5]
head(assay_counts)
```
...
```{r}
sample_metadata <-read_csv("/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/summarized_experiment/sample_metadata.csv")
head(sample_metadata)
colnames(sample_metadata)[1] <- 'sample'
```

```{r}
gene_metadata <-read_csv("/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/summarized_experiment/gene_metadata.csv")
head(gene_metadata)
colnames(gene_metadata)[1] <- 'gene'
head(gene_metadata)
```
